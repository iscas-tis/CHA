...
class ChiselStage extends Stage {
  ...
  final def emitFirrtl(
    ...
    args: Array[String] = Array.empty,
    annotations: AnnotationSeq = Seq.empty): String = {
    //why don't you support Chinese??? !!!
    //ChiselGeneratorAnnotation is a case class, because no new. And it has a default apply function.
    
    /*val Afterexe = execute(Array("-X", "high") ++ args, ChiselGeneratorAnnotation(() => gen) +: annotations)
    println("-------------------------")
    for(temps <- Afterexe)
    {
