...
class Counter(width: Int) extends Module {
  ...
  // chaAssert(this,getTypeTag(this),nname,this.reset)
  ...
  //       {
  //         new chaAnno(Seq(Seq(AtmPropAnno(hh.toTarget)):+Seq(ResetAnno(this.reset.toTarget))))
  //       }
  //     })
...
//   chaAssert(this.reset, ap(countReg(0)) ###(1,-1) *(2,3))
//   chaAssert(this.reset,G ap(countReg(0)) ###(1,-1) *(2,3))
//   chaAssert(this.reset,G ap(countReg(0)) ###(1,-1) *(2,3) U X ap(countReg(0)) ###(1,-1) |-> F ap(countReg(2)))
//   chaAssert(this.reset, ap(countReg(0)) ###(1,-1) |-> F ap(countReg(2)))
//   chaAssert(this.reset, ap(countReg(0)) U ap(countReg(1)) U ap(countReg(0)) )
// //   assume(reset==false)
//   chaAssert(this.reset, |- ap(countReg(0)) -|)
//   chaAssert(this.reset, |- |- ap(countReg(0)) -| -|)
//   chaAssert(this.reset, |- |- ap(countReg(0)) ###(1,-1) -| *(2,3) -| ###(2,3) ap(countReg(1)))
//   chaAssert(this.reset, |- |- ap(countReg(0)) ###(1,-1) -| *(2,3) -| ###(2,3) ap(countReg(1)) U X ap(countReg(2)) *(4,5) |-> G ap(countReg(2)))
//   chaAssert(this.reset, |- |- ap(countReg(0)) ###(1,-1) -| *(2,3) -| U ###(2,3) ap(countReg(1)))
//   chaAssert(this.reset, ap(countReg(0) & !countReg(2)) |-> ###(1,4) ap(countReg(2) ) )
//   chaAssert(this.reset, ap(!countReg(0)) ###(1,-1) ap(countReg(1)))
//   chaAssert(this.reset, ap(!countReg(0)) ###(1,-1) ap(countReg(1)) |-> G X ap(!countReg(0)))
//   chaAssert(this.reset, |- ap(!countReg(0)) ###(1,-1) ap(countReg(1)) ###(2,3) *(1,-1) |->  G X ###(1,-1) ap(countReg(1)) -|)
//   chaAssert(this.reset, ap(!countReg(0)) ###(1,-1) ap(countReg(1)) ###(2,3) *(1,-1) |-> |- G X ###(1,-1) ap(countReg(1)) -|)
//   chaAssert(this.reset, ap(!countReg(0)) ###(1,-1) |- ap(countReg(1)) ###(2,3) -| *(1,-1) |-> G X ap(!countReg(0)))
  
//   chaAssert(this.reset, F |- |- ###(2, 3) ap(countReg(0)) -| *(2,3) | ap(countReg(1)) U G ap(countReg(0)) -| )
  
//   //chaAssert(this.reset, |- F  ###(2, 3) ap(countReg(0)) *(2,3) -| | ap(countReg(1))  U G ap(countReg(0)) )
  
//   chaAssert(this.reset, G |- ###(2, 3) ###(2, 3) ap(countReg(0)) && G ap(countReg(0)) -|)
//   chaAssert(this.reset, F |- |- ###(2, 3) ap(countReg(0)) -| *(2,3) | ap(countReg(1)) U G ap(countReg(0)) -| )
//   chaAssert(this.reset, ap(countReg(0))) 
//   printlnTSeq(###(2, 3) ###(2, 3) ap(countReg(0)) ###(2, 3) ap(countReg(0)) |-> ap(countReg(1)) |-> G F ap(countReg(0)) )
//   chaAssert(Seq(FinalOp(), Leftbraket(), Leftbraket(), NextOp(), GlobalOp(), AtmProp(countReg(0))))
//   chaAssert(this.reset,! ap(countReg(0)) |-> F ! G ap(countReg(0)))
//   chaAssert(this.reset, |- ap(!countReg(0)) -|)
  io.dout := countReg
  // assert(countReg(0))
