...
abstract class EnumType(private val factory: EnumFactory, selfAnnotating: Boolean = true) extends Element {
  ...
  override private[chisel3] def bind(target: Binding, parentDirection: SpecifiedDirection = SpecifiedDirection.Unspecified): Unit = {
    ...
    if (selfAnnotating && isSynthesizable && topBindingOpt.get.isInstanceOf[ConstrainedBinding]) {
      val defAnno = CustomRadixDefAnnotation(enumTypeName, factory.enumValues zip factory.enumNames, width.get)
      if(!Builder.annotations.exists(_.toFirrtl == defAnno))
        annotate(new ChiselAnnotation { def toFirrtl: Annotation = defAnno })
      annotate(new ChiselAnnotation { def toFirrtl: Annotation = CustomRadixApplyAnnotation(toTarget, enumTypeName) })
    }
  }
